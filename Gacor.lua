local Library, SaveManager, InterfaceManager, Mobile = (function()
...
end)() -- ini akhir beta.lua


----------------------------------------------------
-- 🪟 WINDOW SETUP INI SCRIPT AKU
----------------------------------------------------
...
----------------------------------------------------
Home:AddParagraph({
    Title = "WELCOME TO EnemySaga",
    Content = "Welcome to your custom Script Hub! Made with ❤️."
})
Home:AddButton({
    Title = "Check Version",
    Description = "Print current version to console.",
    Callback = function()
        print("[EnemySaga] Version:", Fluent.Version or "Unknown")
    end
})
...
Vuln:AddToggle("SuperInstantFishing", {
    ...
    Default = false,
    Callback = function(Value)
        superInstantEnabled = Value
        if Value then
            Fluent:Notify({
                Title = "Super Instant Fishing",
                Content = "Activated ⚡ Perfect Timing + Anti-Pause enabled",
                Duration = 3
            })
            superInstantThread = task.spawn(function()

                print("=== Auto Fishing (Perfect Timing + Anti-Pause) ===")
                print("Starting in 1 seconds...")

                local autoFishEnabled = true
                local lastCycleTime = tick()
                task.wait(1)
                ...
                task.spawn(function()
                    while task.wait(0.015) do
                        if not superInstantEnabled then break end
                ...
                -- ========================================
                -- WATCHDOG: Anti-Pause System
                -- ========================================
                task.spawn(function()
                    while autoFishEnabled and superInstantEnabled do
                        task.wait(2.5)
                ...
                -- ========================================
                -- MAIN CYCLE (Perfect Timing)
                -- ========================================
                task.spawn(function()
                    while autoFishEnabled and superInstantEnabled do
                        lastCycleTime = tick()

                        -- PHASE 1: CANCEL + CHARGE
                        task.spawn(function()
                            pcall(function()
                                game:GetService("ReplicatedStorage").Packages._Index:FindFirstChild("sleitnick_net@0.2.0").net:FindFirstChild("RF/CancelFishingInputs"):InvokeServer()
                            end)
                        ...
                        task.spawn(function()
                            pcall(function()
                                game:GetService("ReplicatedStorage").Packages._Index:FindFirstChild("sleitnick_net@0.2.0").net:FindFirstChild("RF/ChargeFishingRod"):InvokeServer(1761049514)
                            end)
                        end)
                        ...
                        for i = 1, 3 do
                            task.spawn(function()
                                pcall(function()
                                    game:GetService("ReplicatedStorage").Packages._Index:FindFirstChild("sleitnick_net@0.2.0").net:FindFirstChild("RF/RequestFishingMinigameStarted"):InvokeServer(-0.57, 0.95)
                                end)
                            end)
                            task.wait(instantReelDelay)
                        end
                        ...
                        task.wait(superInstantDelay)
                    end
                end)

                print("ANTI-PAUSE WATCHDOG ACTIVE!")
                print("Will kickstart if paused for 2+ seconds")
            end)
        else
            Fluent:Notify({
                Title = "Super Instant Fishing",
            ...
            if superInstantThread then task.cancel(superInstantThread) end
        end
    end
})

Vuln:AddInput("InstantReelDelayInput", {
    Title = "Instant Reel Delay",
    Default = tostring(instantReelDelay),
    Placeholder = "Default: 0.05",
    Numeric = true,
    Callback = function(Value)
        local num = tonumber(Value)
        if num then
            instantReelDelay = num
            Fluent:Notify({
                Title = "Instant Reel Delay",
                Content = "Updated to " .. num,
                Duration = 3
            })
        end
    end
...
Vuln:AddInput("SuperInstantDelayInput", {
    Title = "Super Instant Cooldown",
    Default = tostring(superInstantDelay),
    Placeholder = "Default: 1",
    Numeric = true,
    Callback = function(Value)
        local num = tonumber(Value)
        if num then
            superInstantDelay = num
            Fluent:Notify({
                Title = "Super Instant Cooldown",
                Content = "Updated to " .. num,
                Duration = 3
